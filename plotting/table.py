...
from plotting.locomotion_scores import errors as ERRORS
from plotting.locomotion_scores import means as MEANS
...
def set_highlights(scores, c=0.95):
    means = [score if type(score) != tuple else score[0] for score in scores]
    cutoff = max(means) * c
...
def format_row(buffer, environment, results):
    ...
    results_str = " & ".join(
        format_result(result, h) for result, h in zip(results, highlights)
    )
...
def format_averages(means):
    prefix = "\\multicolumn{2}{c}{\\bf Average} & "
    highlights = set_highlights(means)
    formatted = " & ".join(
        maybe_highlight(str(mean), h) for mean, h in zip(means, highlights)
    )
...
def format_averages_block(algorithms):
    ...
    formatted_block = f"{formatted} \\hspace{{.6cm}} \\\\ \n"
...
def format_table(algorithms, buffers, environments):
    ...
    table = f"{color_prefix}{table_prefix}{blocks}{table_suffix}"
    return table
