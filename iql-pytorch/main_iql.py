...
def eval_policy(args, iter, video: VideoRecorder, logger: Logger, policy, env_name, seed, mean, std, seed_offset=100, eval_episodes=20):
    ...
    if args.use_wandb:
        wandb.log({
            'eval/lengths_mean': np.mean(lengths),
            'eval/lengths_std': np.std(lengths),
            'eval/returns_mean': np.mean(returns),
            'eval/returns_std': np.std(returns),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--wandb_project', default='iql-pytorch', type=str)
    ...
    if args.use_wandb:
        wandb.init(project=args.wandb_project, entity=args.wandb_entity, name=exp_name, config=args)
    ...
    for t in trange(int(args.max_timesteps)):
        log_dic = policy.train(replay_buffer, args.batch_size, logger=logger)
        if args.use_wandb:
            wandb.log(log_dic, step=t)
        # Evaluate episode
