...
class IQL(object):
    ...
    def update_v(self, states, actions, logger=None):
        ...
        return {'train/value_loss': value_loss.item(), 'train/v': v.mean().item()}
    ...
    def update_q(self, states, actions, rewards, next_states, not_dones, logger=None):
        ...
        return {'train/critic_loss': critic_loss.item(), 'train/q1': q1.mean().item(), 'train/q2': q2.mean().item()}
    ...
    def update_actor(self, states, actions, logger=None):
        ...
        return {'train/actor_loss': actor_loss.item(), 'train/adv': (q-v).mean().item()}
    ...
    def train(self, replay_buffer, batch_size=256, logger=None):
        ...
        # Update
        log_dic = {}
        log_dic.update(self.update_v(state, action, logger))
        log_dic.update(self.update_actor(state, action, logger))
        log_dic.update(self.update_q(state, action, reward, next_state, not_done, logger))
        self.update_target()
