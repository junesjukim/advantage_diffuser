...
def load_diffusion(
    *loadpath, epoch="latest", device="cuda:0", seed=None, n_sample_timesteps=None
):
    ...
    if n_sample_timesteps is not None:
        print(
            "=" * 15
            + "added n_sample_timesteps to diffusion_config  ::: "
            + str(n_sample_timesteps)
            + "=" * 15
        )
        diffusion_config._dict["n_sample_timesteps"] = n_sample_timesteps
