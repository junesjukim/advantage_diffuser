...
class GaussianDiffusion(nn.Module):
    ...
    def register_diffusion_parameters(self):
        ...
        self.sample_sqrt_one_minus_alphas_cumprod = torch.sqrt(
            1.0 - sample_alphas_cumprod
        )
        self.sample_log_one_minus_alphas_cumprod = torch.log(
            1.0 - sample_alphas_cumprod
        )
        self.sample_sqrt_recip_alphas_cumprod = torch.sqrt(1.0 / sample_alphas_cumprod)
        self.sample_sqrt_recipm1_alphas_cumprod = torch.sqrt(
        ...
        self.sample_posterior_log_variance_clipped = torch.log(
            torch.clamp(sample_posterior_variance, min=1e-20)
        )
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        sample_sqrt_recip_alphas_cumprod = self.sample_sqrt_recip_alphas_cumprod.to(
            current_device
        )
        sample_sqrt_recipm1_alphas_cumprod = self.sample_sqrt_recipm1_alphas_cumprod.to(
            current_device
        )
    ...
    def q_posterior(self, x_start, x_t, t):
        ...
        # 필요한 텐서들을 현재 디바이스로 이동
        sample_posterior_mean_coef1 = self.sample_posterior_mean_coef1.to(
            current_device
        )
        sample_posterior_mean_coef2 = self.sample_posterior_mean_coef2.to(
            current_device
        )
        sample_posterior_variance = self.sample_posterior_variance.to(current_device)
        sample_posterior_log_variance_clipped = (
            self.sample_posterior_log_variance_clipped.to(current_device)
        )
        ...
        sample_posterior_log_variance_clipped = extract(
            sample_posterior_log_variance_clipped, t, x_t.shape
        )
        return (
            sample_posterior_mean,
            sample_posterior_variance,
        ...
        if FLOWMATCHING_MODE:
            ...
            x_recon = self.predict_start_from_noise(
                ...
                noise=self.model(
                    x,
                    cond,
                    ((t + 1) * (self.n_timesteps // self.n_sample_timesteps) - 1),
                ),
            )
