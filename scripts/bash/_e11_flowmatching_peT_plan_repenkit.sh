    ...
    for plan_seed in {0..19}; do
      ...
      for i in "${!GPU_DEVICES[@]}"; do
        OMP_NUM_THREADS=24 CUDA_VISIBLE_DEVICES=${GPU_DEVICES[$i]} python scripts/plan_guided.py \
          --dataset ${DATASETS[$i]} \
          --logbase logs \
          --diffusion_loadpath "f:flowmatching/flowmatcher_${prefix_str}_H32_T${n_diffusion_steps}_S${train_seed}" \
          --value_loadpath "f:values/flowmatching_repenkit_H32_T${n_diffusion_steps}_S${value_seed}_d0.99" \
          --horizon 32 \
          --n_diffusion_steps ${n_diffusion_steps} \
          --seed $plan_seed \
          --n_sample_timesteps ${n_sample_timesteps[$i]} \
          --discount 0.99 \
          --prefix ${prefix_path}/TR${train_seed}_VS${value_seed}_PS${plan_seed} > output/flowmatching_plan_${prefix_str}/output_${GPU_DEVICES[$i]}_train${train_seed}_value${value_seed}_plan${plan_seed}.log 2>&1 &
