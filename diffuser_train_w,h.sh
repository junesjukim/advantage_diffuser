...
OMP_NUM_THREADS=24 CUDA_VISIBLE_DEVICES=0 python scripts/train.py --dataset "walker2d-medium-expert-v2" --logbase logs --horizon 4 --n_diffusion_steps 20 --seed 0 --prefix 'diffusion/diffuser'   > output/we_gpu0.log 2>&1 &

# 두 번째 실행: CUDA device 1 사용
OMP_NUM_THREADS=24 CUDA_VISIBLE_DEVICES=1 python scripts/train.py --dataset "walker2d-medium-replay-v2" --logbase logs --horizon 4 --n_diffusion_steps 20 --seed 0 --prefix 'diffusion/diffuser'   > output/wr_gpu1.log 2>&1 &

# 세 번째 실행: CUDA device 2 사용
OMP_NUM_THREADS=24 CUDA_VISIBLE_DEVICES=2 python scripts/train.py --dataset "hopper-medium-expert-v2" --logbase logs --horizon 4 --n_diffusion_steps 20 --seed 0 --prefix 'diffusion/diffuser'   > output/he_gpu2.log 2>&1 &
