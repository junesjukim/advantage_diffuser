...
do
  ...
  for i in "${!GPU_DEVICES[@]}"; do
    OMP_NUM_THREADS=24 CUDA_VISIBLE_DEVICES=${GPU_DEVICES[$i]} python scripts/plan_guided.py \
      --dataset ${DATASETS[$i]} \
      --logbase logs \
      --diffusion_loadpath "f:flowmatching/flowmatcher_repenkit_H32_T${n_diffusion_steps}_S0" \
      --value_loadpath "f:values/flowmatching_H32_T${n_diffusion_steps}_S0_d0.99" \
      --horizon 32 \
      --n_diffusion_steps ${n_diffusion_steps} \
      --seed $seed \
      --n_sample_timesteps ${n_sample_timesteps[$i]} \
      --discount 0.99 \
      --prefix ${prefix_path}/${diffusion_epochs[$i]}/ > output/flowmatching_plan_${prefix_str}/output_${GPU_DEVICES[$i]}_seed_${seed}_E${diffusion_epochs[$i]}.log 2>&1 &
